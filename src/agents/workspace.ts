...
export async function ensureAgentWorkspace(params?: {
  ...
  // Superpack: workspace_bootstrap_before hook — allow plugins to filter/skip files
  const hookRunner = getGlobalHookRunner();
  let templateFiles = [
    { name: DEFAULT_AGENTS_FILENAME, content: agentsTemplate, path: agentsPath },
    { name: DEFAULT_SOUL_FILENAME, content: soulTemplate, path: soulPath },
    { name: DEFAULT_TOOLS_FILENAME, content: toolsTemplate, path: toolsPath },
    { name: DEFAULT_IDENTITY_FILENAME, content: identityTemplate, path: identityPath },
    { name: DEFAULT_USER_FILENAME, content: userTemplate, path: userPath },
    { name: DEFAULT_HEARTBEAT_FILENAME, content: heartbeatTemplate, path: heartbeatPath },
  ];
  let skipBootstrap = false;

  if (hookRunner?.hasHooks("workspace_bootstrap_before")) {
    try {
      const beforeResult = await hookRunner.runWorkspaceBootstrapBefore(
        {
          workspaceDir: dir,
          files: templateFiles.map((f) => ({ name: f.name, content: f.content })),
          isNewWorkspace: isBrandNewWorkspace,
        },
        {},
      );
      if (beforeResult) {
        if (beforeResult.skip) {
          ...
          // Rebuild templateFiles from hook result, preserving path mappings
          const pathMap: Record<string, string> = {};
          for (const f of templateFiles) pathMap[f.name] = f.path;
          templateFiles = beforeResult.files
            .filter((f) => f.name in pathMap)
            .map((f) => ({ name: f.name, content: f.content, path: pathMap[f.name] }));
        }
      }
    } catch {
      // Hook error: proceed with original files
    }
  }
  ...
  if (!skipBootstrap) {
    for (const file of templateFiles) {
      const wrote = await writeFileIfMissing(file.path, file.content);
      if (wrote) {
        filesWritten.push(file.name);
      } else {
        filesSkipped.push(file.name);
      }
    }
  } else {
    filesSkipped.push(...templateFiles.map((f) => f.name));
  }

  // Superpack: workspace_bootstrap_after hook — notify plugins
  if (hookRunner?.hasHooks("workspace_bootstrap_after")) {
    try {
      await hookRunner.runWorkspaceBootstrapAfter(
        { workspaceDir: dir, filesWritten, filesSkipped },
        {},
      );
    } catch {
      // Hook error: swallow
